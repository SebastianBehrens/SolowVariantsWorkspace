...
create_parameter_grid_advanced <- function(ModelCode, np){
    ...
    # create strings of parameters
        # i <- "n"
        # ModelCode <- "BS"
    string_parameterchange_indicator <- paste("ComparingModels_", ModelCode, "_parameterchange_indicator_", partAhelper_1(i), sep = "")
    string_parameterchange_period <- paste("ComparingModels_", ModelCode, "_parameterchange_period_", partAhelper_1(i), sep = "")
    string_parameterchange_valuebefore <- paste("ComparingModels_", ModelCode, "_parameterchange_valuebefore_", partAhelper_1(i), sep = "")
    string_parameterchange_valueafter <- paste("ComparingModels_", ModelCode, "_parameterchange_valueafter_", partAhelper_1(i), sep = "")
...
getRequiredParams <- function(ModelCode){
    out <- case_when(
        ModelCode == "BS" ~ c("B", "alpha", "delta", "n", "s"), 
        ModelCode == "GS" ~ c("g", "alpha", "delta", "n", "s"), 
        ModelCode == "ESSOE" ~ c("B", "alpha", "n", "s", "r"), 
        ModelCode == "ESSRL" ~ c("alpha", "beta", "kappa", "delta", "n", "s", "g", "X"), 
        ModelCode == "ESSRO" ~ c("alpha", "beta", "n", "g", "sE", "s", "delta"), 
        ModelCode == "ESSROL" ~ c("alpha", "beta", "kappa", "delta", "n", "s", "sE", "g", "X"), 
        ModelCode == "ESHC" ~ c("alpha", "phi", "n", "g", "sK", "sH", "delta")
        TRUE ~ NA
    )
