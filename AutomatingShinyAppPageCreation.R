...
createpartA <- function(parameternames, new_abbreviation, startvars){
    ...
    for(aux_parameter in rev(parameternames)){
        ...
        aux_model_math <- pageCreationHelper_modelmath(new_abbreviation)
        # aux_model_math <- gsub(pattern = "\\", replace = "\\\\", x = aux_model_math)
        aux_model_math <- paste0("'", aux_model_math, "'")
        writeLines(gsub(pattern = "aux_modelmath", replace = aux_model_math, x = read_lines("TemplatePartA.R")), "TemplatePartA.R")
...
createPage <- function(parameters, abbreviation, name_of_sim_function, startvars){
    # Create new folder and set as directory (and maybe delete directory before doing that) ---------------------------------
    setwd("/Users/sebastianbehrens/Documents/GitHub/SolowVariantsWorkspace/PageCreation")
    files <- list.files()
    exists_already_indicator <- files %in% abbreviation
    if(any(exists_already_indicator) == TRUE){
      unlink(files[exists_already_indicator], recursive = T)
    }
...
createParameterInterface <- function(parameters, ModelCode, number){
    setwd("/Users/sebastianbehrens/Documents/GitHub/SolowVariantsWorkspace/TabCreation")
    files <- list.files()
    exists_already_indicator <- files %in% paste0("DynamicInterface", ModelCode)
    if(any(exists_already_indicator) == TRUE){
      unlink(files[exists_already_indicator], recursive = T)
    }
    ...
    parameters <- getRequiredParams(ModelCode)
    # ModelCode <- "BS"
    ...
    if(number == 2){
        writeLines(gsub(pattern = "s1", replace = "s2", x = read_lines("TemplatePartA.R")), "TemplatePartA.R")
        writeLines(gsub(pattern = "n1", replace = "n2", x = read_lines("TemplatePartA.R")), "TemplatePartA.R")
