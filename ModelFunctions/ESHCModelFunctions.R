...
ESHC_SS_YpEW <- function(sK, sH, n, g, delta, alpha, phi){(((sK^(1-phi)*sH^phi)/(n + g + delta + n * g))^(1/(1-alpha - phi)))^alpha *(((sK^alpha * sH^(1-alpha))/(n + g + delta + n * g))^(1/(1-alpha- phi)))^phi}
# ESHC_SS_YpEW <- function(HpEW, KpEW, A, alpha, phi){KpEW^alpha * HpEW^phi}
# ESHC_SS_YpW <- function(HpEW, KpEW, A, alpha, phi){A * KpEW^alpha * HpEW^phi}
ESHC_SS_YpW <- function(A, sK, sH, n, g, delta, alpha, phi){A * (((sK^(1-phi)*sH^phi)/(n + g + delta + n * g))^(1/(1-alpha - phi)))^alpha *(((sK^alpha * sH^(1-alpha))/(n + g + delta + n * g))^(1/(1-alpha- phi)))^phi}
