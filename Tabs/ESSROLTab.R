    ...
    sidebarLayout(
      ...
      sidebarPanel(
        ...
        checkboxGroupInput("ESSROL_vtv",
          label = "",
          choices = getModelVars("ESSROL"),
          selected = getModelVars("ESSROL")[1:5]
        ),
        hr(),
        # Periods ---------------------------------
...
# sectiontitle ---------------------------------
numericInput("ESSROL_initparam_alpha", "Alpha", 1/3, step = 0.05),
...
# sectiontitle ---------------------------------
numericInput("ESSROL_initparam_beta", "Beta", 1/3, step = 0.05),
...
# sectiontitle ---------------------------------
numericInput("ESSROL_initparam_kappa", "Kappa", 1/6, step = 0.05),
...
hr(),
      ...
      # Main Panel  ---------------------------------
      mainPanel(
        h1("The Extended Solow Model with Scarce Resources Oil and Land", align = "center"),
        ...
        withMathJax(),
        '$$ \\begin{aligned} Y_t &= K_t^\\alpha  * (A_t * L_t)^{(1- \\beta)} * X^\\kappa * E_t^\\varepsilon : \\alpha + \\beta + \\varepsilon + \\kappa= 1 \\\\ E_t &= s_ER_t\\\\ R_t &= R_{t-1} - E_{t-1}\\\\ K_{t+1} &= s_KY_t + (1-\\delta)K_{t} \\\\ L_{t+1}&=(1+n)L_t \\\\ A_{t+1}&=(1+g)A_t \\\\ \\end{aligned} $$',
        # Visualisation  ---------------------------------
        # textOutput("test"),
        HTML('<div style="height:50px"></div>'),
