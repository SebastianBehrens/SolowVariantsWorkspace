ESEGCozziOneTab <- 
    tabPanel("Extended Solow Model (Endogenous Growth Cozzi Extension)", fluid = TRUE, sidebarLayout(
    # Sidebar Panel  ---------------------------------
...
# sectiontitle ---------------------------------
numericInput("ESEGCozziOne_initparam_phi", "Phi", 1/6, step = 0.05),
...
conditionalPanel(
    ...
    numericInput("ESEGCozziOne_pc_phi_newval", "New Value of Phi", 3/5, step = 0.05)),
hr(),



# sectiontitle ---------------------------------
numericInput("ESEGCozziOne_initparam_lambda", "lambda", 1, step = 0.05),
...
conditionalPanel(
    ...
    numericInput("ESEGCozziOne_pc_lambda_newval", "New Value of lambda", 0.8, step = 0.05)),
hr(),



# sectiontitle ---------------------------------
numericInput("ESEGCozziOne_initparam_rho", "Rho", 0.5, step = 0.1),
...
conditionalPanel(
    ...
    numericInput("ESEGCozziOne_pc_rho_newval", "New Value of Rho", 0.6, step = 0.1)),
hr(),
...
# sectiontitle ---------------------------------
numericInput("ESEGCozziOne_initparam_sR", "Prop. Labor Force in R&D", 0.05, step = 0.05),
...
hr()
    ...
    mainPanel(
        ...
        withMathJax(),
        '$$\\begin{aligned} Y_t &= K_t^\\alpha (A_tL_{Y,t})^{1-\\alpha} \\\\ A_{t+1} &= \\rho A_t s_R^\\lambda + A_t \\\\ K_{t+1}&= sY_t + (1-\\delta)K_t \\\\ L_{t+1}&=(1+n)L_t \\\\ L_t &= L_{A,t} + L_{Y,t} \\\\ L_{A, t} &= s_R * L_t (\\rightarrow L_{Y,t} = (1-s_R * L_t)) \\end{aligned}$$',
        # Visualisation  ---------------------------------
        # textOutput("test"),
